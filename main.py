...
def main(rank, world_size, pend):
    ...
    # instantiate model
    model, optimizer = load_train_objs(vocab_size, rank, learning_rate)
    model.to(rank)
    print(f'rank:{rank}')
    model = DDP(model, device_ids=[rank], find_unused_parameters=True) # move model parameters to gpu if available
    ...
    # train loop
    train(model, optimizer, rank, num_chars, val_data, 
